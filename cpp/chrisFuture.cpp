...
template <typename Type, size_t alignment = alignof(Type)> struct AlignedBuffer
{
...
template <typename Type> struct Promise;

template <typename Type> struct SharedStateBase
{
    ...
    void set(const Type &value)
        requires std::copy_constructible<Type>
    {
        std::call_once(set_flag, static_cast<void (SharedStateBase::*)(const Type &)>(&SharedStateBase::do_set), this,
                       value);
    }
    ...
    void set(Type &&value)
        requires std::move_constructible<Type>
    {
        std::call_once(set_flag, static_cast<void (SharedStateBase::*)(Type &&)>(&SharedStateBase::do_set), this,
                       std::move(value));
    }

    void set(std::exception_ptr ep)
    {
        std::call_once(set_flag, static_cast<void (SharedStateBase::*)(std::exception_ptr)>(&SharedStateBase::do_set),
                       this, std::move(ep));
    }
...
template <typename Type> struct Future
{
    template <typename Functor, typename... Args>
    Future<std::invoke_result_t<std::decay_t<Functor>, std::decay_t<Type>, std::decay_t<Args>...>> then(
        Functor &&functor, Args &&...args)
    {
        ...
        // copy/move semantics?
        state->continuation = [then_state, functor = std::move(functor),
                               ... args = std::forward<Args>(args)](Type value) mutable {
            try
            {
  ...
  private:
    ...
    template <typename Other> friend class Promise;
    using State = std::shared_ptr<SharedStateBase<Type>>;

    Future(State initial_state) noexcept : state(std::move(initial_state))
    {
    }

    Future(const Type &value) : state(std::make_shared<SharedStateBase<Type>>())
    {
        state->set(value);
    }
    Future(Type &&value) : state(std::make_shared<SharedStateBase<Type>>())
    {
...
template <typename Type> struct Promise
{
    Promise() : state(std::make_shared<SharedStateBase<Type>>())
    {
    }
...
template <typename Type> struct PackagedTask
{
};

template <typename Functor, typename... Args>
[[nodiscard]]
Future<std::invoke_result_t<std::decay_t<Functor>, std::decay_t<Args>...>> async(Functor &&functor, Args &&...args)
{
    using Type = std::invoke_result_t<std::decay_t<Functor>, std::decay_t<Args>...>;
    Promise<Type> promise;
    auto future = promise.get_future();
    std::thread t([promise = std::move(promise), function = std::forward<Functor>(functor),
...
int main(const int argc, const char *argv[])
{
    ...
    auto future = async([] { return 42; })
                      ...
                      .then([](float x) { return double(x / 6); });
    auto second_future = future;
    second_future.then([](double d) { return int(d); });
