...
template <typename T>
Vector<T>::Vector() : m_size{0}, m_capacity{1}, m_container(std::make_unique<T[]>(1)), p_last_el{m_container.get()}
{
}

template <typename T>
Vector<T>::Vector(std::size_t newm_size)
    : m_container{std::make_unique<T[]>(m_capacity)}, m_size{newm_size}, m_capacity{newm_size},
...
Vector<T>::Vector(std::size_t newm_size, T data)
    : m_container{std::make_unique<T[]>(m_capacity)}, m_data{data}, m_size(newm_size),
      p_last_el{m_container.get() + newm_size - 1}
{
}
...
Vector<T>::Vector(std::size_t newm_size, T data, std::size_t cap)
    : m_container{std::make_unique<T[]>(m_capacity)}, m_data{data}, m_size(newm_size), m_capacity{cap},
      p_last_el{m_container.get() + newm_size - 1}
{
}
...
template <typename T> void Vector<T>::double_capacity()
{
    m_capacity *= 2;
...
template <typename T> void Vector<T>::insert(T val)
{
