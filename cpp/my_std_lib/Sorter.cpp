...
class Sorter
{
  public:
    ...
    /// @tparam T
    /// @param  vec
    template <typename T> static void print(std::vector<T> &vec)
    {
        for (int &el : vec)
            std::cout << el << " ";
        std::cout << std::endl;
    ...
    template <typename T> static void insertionSort(std::vector<T> &vec)
    {
        int n = vec.size();
        for (int i = 1; i < n; i++)
        {
    ...
    /// @tparam T
    /// @param  vec
    template <typename T> static void selectionSort(std::vector<T> &vec)
    {
    ...
    template <typename T> static std::vector<T> quickSort(std::vector<T> &vec)
    {
        if (vec.size() < 2)
            return vec;
        int n = vec.size();
  ...
  private:
    ...
    template <typename T>
    static typename std::vector<T>::iterator _smallest(typename std::vector<T>::iterator it,
